...
def override_forward(self):
    ...
    def forward(
        ...
        iter_cur=0, save_kv=True, test=0
    ):
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if test:
                ...
                # 检查 upsample_block 的设备情况
                for name, param in upsample_block.named_parameters():
                    print(f"Layer: {name}, Device: {param.device}")

                # 检查 CrossAttention 的设备
                if hasattr(upsample_block, "transformer_blocks"):
                    for block in upsample_block.transformer_blocks:
                        if hasattr(block, "attn1"):
                            print(f"Query Device: {block.attn1.q.weight.device}")
                            print(f"Key Device: {block.attn1.k.weight.device}")
                            print(f"Value Device: {block.attn1.v.weight.device}")
...
class DragPipeline(StableDiffusionPipeline):
    ...
    def step(
        ...
        if eta > 0:
            one_minus_ab_t      = (1 - alpha_prod_t).clamp(min=1e-12)
            one_minus_ab_t_prev = (1 - alpha_prod_t_prev).clamp(min=1e-12)
            frac = (one_minus_ab_t_prev / one_minus_ab_t).clamp(min=0.0)
            inner = (1 - (alpha_prod_t / alpha_prod_t_prev).clamp(min=0.0, max=1.0)).clamp(min=0.0)
            sigma_t = eta * (frac.sqrt() * inner.sqrt())
        else:
            sigma_t = x.new_zeros(())
        if mask is not None:
            if mask.shape[-2:] != x.shape[-2:]:
                mask = F.interpolate(mask.to(x.dtype), size=x.shape[-2:], mode="nearest")
        dir_coeff = (1 - alpha_prod_t_prev - sigma_t** 2 * mask).clamp(min=0.0).sqrt()
        x_prev_mean = alpha_prod_t_prev.sqrt() * pred_x0 + dir_coeff * model_output
        if eta > 0:
            if z is None:
                z = torch.randn_like(x)
            if mask is None:
                # 无 mask 时等价于标准 DDIM 随机项
                x_prev = x_prev_mean + sigma_t * z
            else:
                # 仅在 mask==1 的位置加随机项；未掩膜处不加
                # 确保 mask 形状为 (B,1,H,W)
                if mask.shape[-2:] != x.shape[-2:]:
                    mask = F.interpolate(mask.to(x.dtype), size=x.shape[-2:], mode="nearest")
                x_prev = x_prev_mean + sigma_t * (z * mask)
        else:
            x_prev = x_prev_mean
    ...
    def __call__(
        ...
        **kwds):
        ...
        for i, t in enumerate(tqdm(self.scheduler.timesteps, desc="DDIM Sampler")):
            ...
            # predict the noise
            noise_pred = self.unet(model_inputs, t, encoder_hidden_states=text_embeddings, iter_cur=i, save_kv=False, test=0)
            ...
            latents = self.step(noise_pred, t, latents, eta, mask)[0]
            # breakpoint()
            # latents = self.step(noise_pred, t, latents, return_dict=False)[0]
        ...
        **kwds):
        ...
        for i, t in enumerate(tqdm(reversed(self.scheduler.timesteps), desc="DDIM Inversion")):
            ...
            noise_pred = self.unet(model_inputs, 
                                   ...
                                   iter_cur=len(self.scheduler.timesteps)-i-1, 
                                   save_kv=save_kv) # change this for memory
            if guidance_scale > 1.:
                noise_pred_uncon, noise_pred_con = noise_pred.chunk(2, dim=0)
                noise_pred = noise_pred_uncon + guidance_scale * (noise_pred_con - noise_pred_uncon)
            ...
            iter_cur_list.append(len(self.scheduler.timesteps)-i-1)
        ...
        if return_intermediates:
            ...
            return latents, latents_list, timesteps_list, iter_cur_list, text_embeddings
        return latents
