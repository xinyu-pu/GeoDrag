...
def get_down_block(
    ...
    elif down_block_type == "CrossAttnDownBlock2D":     # change this
...
def get_up_block(
    ...
    elif up_block_type == "CrossAttnUpBlock2D":         # change this
        if cross_attention_dim is None:
...
# change this UNetMidBlock2DCrossAttn
...
class CrossAttnDownBlock2D(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            ...
            # in drag, dual_cross_attention is False
            if not dual_cross_attention:
                # so change this
                attentions.append(
                    Transformer2DModel(
        ...
        for i, (resnet, attn) in enumerate(blocks):
            if self.training and self.gradient_checkpointing:
                ...
                def create_custom_forward(module, return_dict=None):
                    def custom_forward(*inputs):
                        if return_dict is not None:
                            return module(*inputs, return_dict=return_dict) # set iter_cur or not?
                        else:
                            return module(*inputs)
