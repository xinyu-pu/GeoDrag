...
# print("================change attention for drag=================")
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        # iter_cur save_kv updown are add for drag memory
        self.updown=updown
        self.isDrag = isDrag # change this 
        super().__init__()
...
class AttnProcessor:
    ...
    def __call__(
        ...
        # change this --------------------------------------------
        # print('relace kv in attention_processor AttnProcessor')
        ...
        if attn.updown == 'up' and not save_kv:
            if iter_cur>=5: 
                key_ref = attn.buffer_key[iter_cur].to(query.device, dtype=query.dtype)
                value_ref = attn.buffer_value[iter_cur].to(query.device, dtype=query.dtype)
...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        if attention_mask is not None:
            ...
            attn_mask (optional Tensor) – Attention mask; shape(N,...,L,S).
              Two types of masks are supported. 
              A boolean mask where a value of True indicates that the element should take part in attention. 
              A float mask of the same type as query, key, value that is added to the attention score.

              when is bool: false is not to attention, true is attention
            '''
        ...
        # change this --------------------------------------------
        # print('relace kv in attention_processor AttnProcessor2_0')
        # print("in attention_processor KV shape: ", query.shape, key.shape, value.shape)
        if attn.updown == 'up' and save_kv:     # inverse
            # print("attnprocessor2_0 save_kv iter_cur", save_kv, iter_cur)
            if not hasattr(attn, 'buffer_key'):
                attn.buffer_key = {}
                attn.buffer_value = {}
            attn.buffer_key[iter_cur] = key.cpu()
            attn.buffer_value[iter_cur] = value.cpu()
        if attn.updown == 'up' and not save_kv:         # sampling
            # print("iter_cur",iter_cur)
            if iter_cur>=0 and hasattr(attn, 'buffer_key'): 
                # print("attnprocessor2_0 not save_kv iter_cur", save_kv, iter_cur)
                # masactrl's mutual self-attention
                key_ref = attn.buffer_key[iter_cur].to(query.device, dtype=query.dtype)
                value_ref = attn.buffer_value[iter_cur].to(query.device, dtype=query.dtype)
                
                key = key_ref.repeat(2,1,1)
                value = value_ref.repeat(2,1,1)
                # print("do iter_cur", iter_cur)

                # rlc+kv copy
                # key = torch.cat([key[0:1], key_ref], dim=0)
                # value = torch.cat([value[0:1], value_ref], dim=0)

                # mean + cat
                # k_m = (key_ref+key)/2
                # v_m = (value_ref+value)/2
                # key = torch.cat([k_m, key_ref.repeat(2,1,1)], dim=1)
                # value = torch.cat([v_m, value_ref.repeat(2,1,1)], dim=1)

                # mean
                # k_m = (key_ref+key)/2
                # v_m = (value_ref+value)/2
                # key = k_m.repeat(2,1,1)
                # value = v_m.repeat(2,1,1)
        ...
        # if attn.updown == 'down' and save_kv:
        #     # print("attnprocessor2_0 save_kv iter_cur", save_kv, iter_cur)
        #     if not hasattr(attn, 'buffer_key'):
        #         attn.buffer_key = {}
        #         attn.buffer_value = {}
        #     attn.buffer_key[iter_cur] = key.cpu()
        #     attn.buffer_value[iter_cur] = value.cpu()
        # if attn.updown == 'down' and not save_kv:
        #     if iter_cur>=0: 
        #         # # print("attnprocessor2_0 not save_kv iter_cur", save_kv, iter_cur)
        #         # masactrl's mutual self-attention
        ...
        # same effect with attn_util.py override_attn_proc_forward  q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=h), (q, k, v))


        # the output of sdp = (batch, num_heads, seq_len, head_dim)
        # TODO: add support for attn.scale when we move to Torch 2.1
        # print("do attention in attention_processor")    # when inversion
        hidden_states = F.scaled_dot_product_attention(
            query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        )
        # print('qkv', query.shape, key.shape, value.shape, hidden_states.shape)
